...
void traverse_and_visit(Set set ,BTreeNode node ,VisitFunc visit) {
	if (node == NULL) {
		return;
	}

	for (int i = 0; i < node->count; i++) {
		traverse_and_visit(set ,node->children[i] ,visit);
		visit(node->set_nodes[i]->value);
	}
	traverse_and_visit(set ,node->children[node->count] ,visit);
...
void set_visit(Set set, VisitFunc visit) {
	...
	*/
	traverse_and_visit(set ,set->root ,visit);
}